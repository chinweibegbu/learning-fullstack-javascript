...
function createArticle(url, request) {
  ...
  if (requestArticle && requestArticle.title && requestArticle.url &&
    ...
    // Add article ID to user's article IDs array
...
function createComment(url, request) {
   ...
   * 1. Exists
   * 2. Has a body
   * 3. Has an article ID & the article ID exists in the DB
   * 4. Has a username & the user exists in the DB
   */
  if (requestComment && requestComment.body &&
    ...
    // Add comment ID to user's comment IDs array
    database.users[comment.username].commentIds.push(comment.id);
    // Add comment ID to article's comment IDs array
...
function updateComment(url, request) {
  ...
  if (!id || !requestComment) {
    ...
    // NOTE: id, username and articleId are NOT editable
...
function deleteComment(url, request) {
  ...
  if (savedComment) {
    ...
    // NOTE: We have already saved it in 'savedComment'
    database.comments[id] = null;

    // Get the commentId array of the user
    const userCommentIds = database.users[savedComment.username].commentIds;
    // Remove the comment ID from the user's commentId array
    userCommentIds.splice(userCommentIds.indexOf(id), 1);

    // Get the commentId array of the article
    const articleCommentIds = database.articles[savedComment.articleId].commentIds;
    // Remove the comment ID from the article's commentId array
