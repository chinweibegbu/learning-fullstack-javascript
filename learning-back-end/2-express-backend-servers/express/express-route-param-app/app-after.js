...
// Add your code here:
app.param('snackId', (req, res, next, snackIdAsString) => {
    const snackId = Number(snackIdAsString);
    const foundIndex = vendingMachine.findIndex((snackDetails) => snackDetails.id === snackId);
    if (foundIndex !== -1) {
        req.snackIndex = foundIndex;
        next();
    } else {
